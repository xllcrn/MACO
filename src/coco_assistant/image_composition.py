...
class ImageComposition():
    ...
    def _validate_and_process_backgrounds(self):
        ...
        for super_category_dir in self.backgrounds_dir.iterdir():
            if not super_category_dir.is_dir():
                warnings.warn(f'file found in backgrounds directory (expected scene directories), ignoring: {super_category_dir}')
                continue
    ...
    def _compose_images(self, foregrounds, background_path):
        ...
        background = background.resize(new_size, resample=Image.Resampling.BICUBIC)
        ...
        for j, fg in enumerate(foregrounds):
            fg_path = fg['foreground_path']
            # min_x, max_x = listx[j]
            # min_y, max_y = listy[j]
            # min_x_position = min_x*composite.size[0]
            # max_x_position = max_x*composite.size[0]
            # min_y_position = min_y*composite.size[1]
            # max_y_position = max_y*composite.size[1]

            # Perform transformations
            fg_image = self._transform_foreground(scale, j, fg, fg_path, composite)

            # # Choose a random x,y position for the foreground
            # max_x_position = max_x*composite.size[0] - fg_image.size[0]
            # max_y_position = max_y*composite.size[1] - fg_image.size[1]
            # if (max_x_position < 0 or max_y_position < 0):
            #     new_size = (int(composite.size[0] * 0.99), int(composite.size[1] * 0.99))
            #     fg_image = fg_image.resize(new_size, resample=Image.Resampling.BICUBIC)
            # max_x_position = max_x*composite.size[0] - fg_image.size[0]
            # max_y_position = max_y*composite.size[1] - fg_image.size[1]
            ...
            while (i_loop and nb_loop<max_loop):
                ...
                x_low = x + fg_image.size[0]
                y_low = y + fg_image.size[1]
                pix = np.array(composite_mask)
                epsilon = min(x, y, composite.size[0]-x_low, composite.size[1]-y_low, int(0.1*fg_image.size[0]))
                epsilonx = random.randint(0, epsilon)
                epsilony = random.randint(0, epsilon)
                square = pix[x-epsilonx:x+fg_image.size[0]+epsilonx,
                         y-epsilony:y+fg_image.size[1]+epsilony:]
                nb_loop += 1
                if (np.sum(square)>0):
                    continue
                else:
            ...
            # assert max_x_position >= min_x_position , f"min is superior to max {max_x_position,min_x_position}"
    ...
    def _transform_foreground(self, scale, position, fg, fg_path, composite):
        # Open foreground and get the alpha channel
        fg_image = Image.open(fg_path)
        # rescaling des trash pour qu'ils aient les dimensions homogenes et coherentes...
        if("Mask" in str(fg_path).split(os.sep) ):
            new_lengthx = scale * composite.size[0]
            new_lengthy = scale * composite.size[1]
        #     if (fg_image.size[0]>fg_image.size[1]):
        #         scale = 0.5*(160./fg_image.size[0] + 80./fg_image.size[1])
        #     else:
        #         scale = 0.5*(80./fg_image.size[0] + 160./fg_image.size[1])
        #     scale = random.uniform(0.4, 0.8)
        elif("Cigarette" in str(fg_path).split(os.sep)):
           new_lengthx = 0.3 * scale * composite.size[0]
           new_lengthy = 0.3 * scale * composite.size[1]
        #     if (fg_image.size[0]>fg_image.size[1]):
        #         scale = 0.5*(45./fg_image.size[0] + 15./fg_image.size[1])
        #     else:
        #         scale = 0.5*(15./fg_image.size[0] + 45./fg_image.size[1])
        # if("container" in str(fg_path).split(os.sep) ):
        ...
        fg_image = fg_image.rotate(angle_degrees, resample=Image.Resampling.BICUBIC, expand=True)
