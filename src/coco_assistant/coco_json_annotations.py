...
class SceneJsonUtils():
    """ Creates a category object to describe a COCO dataset
    """
    def create_coco_scene(self, category_id, name):
        category = dict()
        category['id'] = category_id
        category['name'] = name
...
class ImageJsonUtils():
    ...
    def create_coco_image(self, image_path, image_id, image_license):
        ...
        width, height = image_file.shape[1], image_file.shape[0]
...
class AnnotationJsonUtils():
    ...
    def create_coco_annotations(self, image_mask_path, image_id, category_ids):
        ...
        self.mask_image = cv2.imread(str(image_mask_path))
        # self.mask_image = self.mask_image.convert('RGB')
        self.width, self.height = self.mask_image.shape[1], self.mask_image.shape[0]
    ...
    def create_scene_coco_annotations(self, image_mask_scene, image_id, scene_category_ids):
        """ Takes a pixel-based RGB image mask and creates COCO annotations.
        Args:
            image_mask_path: a pathlib.Path to the image mask
            image_id: the integer image id
        ...
        # Set class variables
        self.image_id = image_id
        self.scene_category_ids = scene_category_ids
        self.image_mask_scene = image_mask_scene

        # Create annotations from the masks
        self._create_scene_annotations()

        return self.scene_annotations
    ...
    def _create_scene_annotations(self):
        # Creates annotations for each isolated mask
        ...
        annotation = dict()
        annotation['image_id'] = self.image_id
        annotation['background_ids'] = [self.scene_category_ids[self.image_mask_scene]]

        # Finally, add this annotation to the list
        self.scene_annotations.append(annotation)
...
class CocoJsonCreator():
    ...
    def create_scene_categories(self):
        """ Creates the "scene_categories" portion of the COCO json
        Returns:
            categories: category objects that become part of the final json
            category_ids_by_name: a lookup dictionary for category ids based
                on the name of the category
        ...
        look_up_table = dict()
        look_up_table["Clean"] = 0
        look_up_table["Indoor, Man-made"] = 1
        look_up_table["Pavement"] = 2
        look_up_table["Sand, Dirt, Pebbles"] = 3
        look_up_table["Trash"] = 4
        look_up_table["Vegetation"] = 5
        look_up_table["Water"] = 6
        look_up_table["City"] = 7

        scene_category = self.mask_definitions['scene_background']
        for category_name in scene_category:
            category_id = look_up_table[category_name]
            categories.append(cju.create_coco_scene(category_id, category_name))
            category_ids_by_name[category_name] = category_id
    ...
    def create_scene_annotations(self, category_ids_by_name):
        """ Creates the list of images (in json) and the annotations for each
            image for the "image" and "annotations" portions of the COCO json
        """
        iju = ImageJsonUtils()
        ...
        # For each mask definition, create scene annotations
        for file_name, mask_def in tqdm(self.mask_definitions['masks'].items()):
            # Create a coco image json item
            image_path = Path(self.dataset_dir) / file_name
            mask_path = Path(self.dataset_dir) / mask_def['mask']
            mask_scene = mask_def['scene_background']

            # Create a dict of category ids keyed by rgb_color
            # category_ids_by_rgb = dict()
            # for scene_category in mask_def['scene_background']:
            #     category_ids_by_rgb[rgb_color] = category_ids_by_name[scene_category]
            annotation_obj = aju.create_scene_coco_annotations(mask_scene, image_id, category_ids_by_name)
            annotation_objs += annotation_obj # Add the new annotations to the existing list
            image_id += 1
    ...
    def main(self, args):
        ...
        scene_categories, scene_category_ids_by_name = self.create_scene_categories()
        scene_annotations = self.create_scene_annotations(scene_category_ids_by_name)
