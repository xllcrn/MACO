class CatRemapper:
    def __init__(self, cat1, cat2):
        ...
        for c in self.refcat:
            c["supercategory"] = c["supercategory"].lower()
            c["name"] = c["name"].lower()
    ...
    def remap_cats(self):
        ...
        if diff:
            ...
            for i, d in enumerate(diff):
                # newcat_dict[self.cat2[d][0]] = [len(c1) + i + 1, self.cat2[d][1]]
                newcat_dict[self.cat2[d][0]] = len(c1) + i + 1
                self.result[d] = [len(c1) + i + 1, self.cat2[d][1]]
                del self.cat2[d]

            for k, v in self.result.items():
                res.append({"supercategory":v[1],"id": v[0], "name": k})

        for k, v in self.cat1.items():
            try:
                # overlap_dict[self.cat2[k][0]] = v
                overlap_dict[self.cat2[k][0]] = v[0]
            except KeyError:
                continue
    ...
    def generate_id_map(self, cat):
        ...
        """
        c = [[i["name"].lower(), i["id"], i["supercategory"].lower()] for i in cat]
        keys = [i[0] for i in c]
        values = [[i[1],i[2]] for i in c]
